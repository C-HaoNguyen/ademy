...
- `dto/CategoryDto.java` (phẳng) → gộp vào `category/dto/` (đã có `dto/category/CategoryRequest.java` — 2 chỗ cho cùng domain, gộp lại).
...
### Phase 2: Package restructure + Service layer skeleton

- **Goal**: Có ranh giới module rõ ràng và nơi đặt business logic trước khi rewrite bất kỳ domain nào.
- **Scope**: Di chuyển code hiện có từ cấu trúc theo layer (`controller/`, `dto/`, `entity/`, `repository/`) sang package theo feature, **đúng theo [Target Project Structure & Organization Rules](#target-project-structure--organization-rules) mục B** (không tự quyết định cấu trúc tại chỗ); tạo Service class cho mỗi feature, **di chuyển logic từ controller vào service nguyên trạng** (không sửa logic ở phase này, chỉ di chuyển).
- **Dependencies**: Phase 1.
- **Changes required**:
  - Move file theo feature package `{auth,user,category,course,enrollment,payment,common,security,seeder}` (thêm `category` — bị thiếu ở bản kế hoạch gốc, xem mục B); `security/` và `seeder/` giữ nguyên là top-level, **không** gộp vào `common/`.
  - `course/lesson/` — submodule chứa `Lessons`, `LessonProgress` (2 entity hiện đang phẳng, chưa thuộc module nào).
  - Di chuyển DTO theo đúng bảng "Vị trí DTO hiện có" ở mục B: `CategoryDto` → `category/dto/` (gộp với `CategoryRequest` đã có sẵn ở đó); `InstructorDto` → `course/dto/`; `EnrollRequest` → `enrollment/dto/`; `PaymentRequest`/`PaymentResponse` → `payment/dto/`.
  - Controller gọi service thay vì gọi repository trực tiếp; sửa luôn field injection `public`/thiếu `final` (`AdminController`, `CategoryController`) thành `private final` nhất quán trong lúc move (không đổi logic, chỉ đổi visibility — an toàn để gộp vào phase move).
  - Gộp CORS: xóa `WebConfig.java`, chỉ giữ cấu hình CORS trong `SecurityConfig.java` (2 cấu hình hiện tại giá trị y hệt nhau — xóa không đổi hành vi quan sát được).
  - Giữ nguyên hành vi API 100%.
- **Modules/files**: Toàn bộ `com.example.academic_management_api.{controller,dto,entity,repository}` → tổ chức lại theo đúng cây thư mục ở mục B (Target Project Structure).
- **Existing behavior cần preserve**: Mọi endpoint hiện có phải trả về response giống hệt trước/sau di chuyển — đây là refactor cơ học thuần túy, không đổi logic.
- **Migration concerns**: Rủi ro lớn nhất là lẫn sửa logic vào cùng lúc di chuyển file — phải tách biệt commit "move" khỏi "change behavior".
- **Tests/verification**: Nếu Phase 19 (test coverage) chưa chạy, verify thủ công qua Postman/curl từng endpoint trước/sau; nếu có thể, viết smoke test tối thiểu trước khi di chuyển để so sánh.
- **Exit criteria**: Toàn bộ endpoint hiện có hoạt động y hệt trước phase; không còn controller gọi trực tiếp repository; cấu trúc package khớp 100% mục B (Target Project Structure); `WebConfig.java` đã xóa, `academic-management-api.iml` đã xóa khỏi git (Cleanup actions mục E).
- **Trace**: ADR-002.

### Phase 3: Global exception handling + Bean Validation

- **Goal**: Chuẩn hóa response lỗi, chặn input không hợp lệ nhất quán.
- **Scope**: Thêm `@ControllerAdvice` ánh xạ exception nghiệp vụ (NotFound/Forbidden/Conflict/ValidationFailed) sang response chuẩn; bổ sung `@Valid` còn thiếu ở `LoginRequest`, `EnrollmentRequest`, `PaymentRequest`.
- **Dependencies**: Phase 2 (cần service layer tồn tại để ném exception nghiệp vụ đúng chỗ).
- **Changes required**: Class exception nghiệp vụ tối thiểu (NotFoundException, ForbiddenException, ConflictException); `GlobalExceptionHandler`; thêm annotation `@Valid` vào 3 controller còn thiếu.
- **Modules/files**: `common/exception/`, `auth/controller/AuthController.java`, `enrollment/controller/EnrollmentController.java`, `payment/controller/PaymentController.java`.
- **Existing behavior cần preserve**: Response thành công không đổi; chỉ đổi hình dạng response lỗi (trước đây không nhất quán, nay chuẩn hóa — cần thông báo rõ đây là breaking change nhỏ về format lỗi, không phải breaking change về logic).
...
- **Tests/verification**: Test case gửi request thiếu field bắt buộc ở 3 endpoint vừa thêm `@Valid`, xác nhận trả 400 với message rõ ràng thay vì lỗi khác/500.
- **Exit criteria**: Mọi lỗi "not found"/"forbidden" trả đúng HTTP status, không còn `RuntimeException` trần lộ 500; 3 endpoint thiếu validate đã có `@Valid`.
